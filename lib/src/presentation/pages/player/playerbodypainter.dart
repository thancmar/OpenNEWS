...
class PlayerBodyPainter extends CustomClipper<Path> {
  ...
  final GlobalKey key;
  const PlayerBodyPainter({required this.index, required this.context, required this.key});
  ...
  getClip(Size size) {
    ...
    final RenderBox renderBoxNavbar = key.currentContext!.findRenderObject() as RenderBox;
    path.lineTo(0.0, 0.0);
    print("render");
    print(size.height);
    ...
    path.cubicTo(leftControlPoint1, size.height - renderBoxNavbar.size.height, leftControlPoint2, controlHeight + (size.height - renderBoxNavbar.size.height), centerPoint,
        controlHeight + size.height - renderBoxNavbar.size.height);
    path.cubicTo(rightControlPoint1, controlHeight + size.height - renderBoxNavbar.size.height, rightControlPoint2, size.height - renderBoxNavbar.size.height, endofBezier,
